...
// Reverse geocoding function to fetch and format the address
...
    
    if (results.length > 0) {
      const addressData = results[0].data.address;

      // Check if the location is in Kenya
      if (addressData.country !== 'Kenya') {
        toast.error('Please select a location within Kenya.');
        setAddress('');
        return;
      }

      // Build the formatted address string with the specified components
      const parts = [];
...
      if (addressData.county) parts.push(addressData.county);
...
          className="px-3 py-2 mt-1 flex-1 border-3 border-bgColor hover:border-bgColorhover rounded-md outline-none focus:ring focus:ring-bgColorhover "
          value={address}
